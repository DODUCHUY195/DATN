...
const CLEANUP_MINUTES = 30;
const SLOT_STEP_MINUTES = 5;
const SCHEDULE_TEMPLATE_STORAGE_KEY = "admin-showtime-day-templates";
...
function loadScheduleTemplates() {
  ...
  try {
    const raw = window.localStorage.getItem(SCHEDULE_TEMPLATE_STORAGE_KEY);
...
function getShowtimeRoomId(showtime) {
  return String(showtime.roomId ?? showtime.Room?.id ?? "");
}
...
function findNextAvailableSlotAfterConflict({
  showtimes,
  roomId,
  preferredStartTime,
  movieDurationMinutes,
  ...
  if (!roomId || !preferredStartTime || !movieDurationMinutes) {
    return {
      ok: false,
      message: "Thiếu phòng, giờ bắt đầu hoặc thời lượng phim.",
    };
  }

  let candidateStart = new Date(preferredStartTime);
  ...
  const targetDate = toDateInputValue(candidateStart);

  const roomShowtimes = [...(showtimes || [])]
    .filter((showtime) => getShowtimeRoomId(showtime) === String(roomId))
    .filter((showtime) => isSameLocalDate(showtime.startTime, targetDate))
    .filter((showtime) => showtime.id !== editingShowtimeId)
    .sort((a, b) => new Date(a.startTime) - new Date(b.startTime));

  let lastConflict = null;
  let guard = 0;

  while (guard < roomShowtimes.length + 5) {
    guard += 1;

    const candidateEnd = addMinutes(candidateStart, movieDurationMinutes);
    const candidateOccupiedEnd = addMinutes(candidateEnd, CLEANUP_MINUTES);

    if (toDateInputValue(candidateEnd) !== targetDate) {
      return {
        ok: false,
        message: "Không thể tự chọn giờ vì suất chiếu sẽ bị qua ngày hôm sau.",
      };
    }

    const conflictedShowtime = roomShowtimes.find((showtime) => {
      const existingStart = new Date(showtime.startTime);
      const existingEnd = new Date(showtime.endTime);
    ...
    lastConflict = conflictedShowtime;

    const conflictedEnd = new Date(conflictedShowtime.endTime);
    candidateStart = addMinutes(conflictedEnd, CLEANUP_MINUTES);
  }

  return {
    ok: false,
    message: "Không tìm được khung giờ trống phù hợp trong ngày.",
  };
}

function validateScheduleTemplateDraft({ draft, movies, rooms }) {
  const general = [];
  const itemErrors = {};
  const normalizedItems = [];
  const allowedStatuses = ["ACTIVE", "INACTIVE"];

  const addItemError = (itemId, message) => {
  ...
  if (!name) {
    general.push("Vui lòng nhập tên mẫu lịch chiếu.");
  }

  if (!Array.isArray(draft.items) || draft.items.length === 0) {
    general.push("Vui lòng thêm ít nhất 1 suất chiếu trong mẫu.");
  }
  ...
  draft.items.forEach((item, index) => {
    ...
    const movie = movies.find(
      (movieItem) => String(movieItem.id) === String(item.movieId),
    );

    const room = rooms.find(
      (roomItem) => String(roomItem.id) === String(item.roomId),
    );
    ...
    if (!item.movieId) {
      addItemError(itemId, `${rowLabel}: Vui lòng chọn phim.`);
    } else if (!movie) {
      addItemError(itemId, `${rowLabel}: Phim không tồn tại.`);
    }

    if (!item.roomId) {
      addItemError(itemId, `${rowLabel}: Vui lòng chọn phòng.`);
    } else if (!room) {
      addItemError(itemId, `${rowLabel}: Phòng không tồn tại.`);
    }

    if (!item.startTime) {
      addItemError(itemId, `${rowLabel}: Vui lòng nhập giờ bắt đầu.`);
    } else if (startMinutes === null) {
      addItemError(
        itemId,
        `${rowLabel}: Giờ bắt đầu không hợp lệ. Định dạng đúng là HH:mm.`,
      );
    }

    if (!Number.isFinite(basePrice)) {
      addItemError(itemId, `${rowLabel}: Giá vé phải là số.`);
    } else if (basePrice < 0) {
      addItemError(itemId, `${rowLabel}: Giá vé không được nhỏ hơn 0.`);
    }

    if (!allowedStatuses.includes(status)) {
      addItemError(itemId, `${rowLabel}: Trạng thái không hợp lệ.`);
    }

    const durationMinutes = Number(movie?.durationMinutes || 0);

    if (movie && durationMinutes <= 0) {
      addItemError(
        itemId,
        `${rowLabel}: Phim "${movie.title}" chưa có thời lượng hợp lệ.`,
      );
    }

    if (startMinutes !== null && durationMinutes > 0) {
      const endMinutes = startMinutes + durationMinutes;
      const occupiedEndMinutes = endMinutes + CLEANUP_MINUTES;

      if (endMinutes > 24 * 60) {
        addItemError(itemId, `${rowLabel}: Suất chiếu bị qua ngày hôm sau.`);
      }

      if (occupiedEndMinutes > 24 * 60) {
        addItemError(
          itemId,
          `${rowLabel}: Suất chiếu cộng thêm ${CLEANUP_MINUTES} phút dọn vệ sinh bị qua ngày hôm sau.`,
        );
      }
  ...
  for (let i = 0; i < normalizedItems.length; i += 1) {
    for (let j = i + 1; j < normalizedItems.length; j += 1) {
      ...
      const next = normalizedItems[j];

      if (current.roomId !== next.roomId) continue;

      const isOverlap =
        current.startMinutes < next.occupiedEndMinutes &&
        current.occupiedEndMinutes > next.startMinutes;

      if (isOverlap) {
        addItemError(
          current.id,
          `${current.rowLabel}: Bị trùng giờ với ${next.rowLabel} trong cùng phòng.`,
        );

        addItemError(
          next.id,
          `${next.rowLabel}: Bị trùng giờ với ${current.rowLabel} trong cùng phòng.`,
        );
      }
    }
  }
  ...
  return {
    valid: general.length === 0 && !hasItemErrors,
    general,
    itemErrors,
  };
}
...
export default function AdminShowtimesPage() {
  ...
  const [applyTemplateDate, setApplyTemplateDate] = useState(() =>
    toDateInputValue(new Date()),
  );
  ...
  const selectedApplyTemplate = useMemo(
    () =>
      scheduleTemplates.find(
        (template) => String(template.id) === String(applyTemplateId),
      ),
    [scheduleTemplates, applyTemplateId],
  );
  ...
  const conflictingShowtimes = useMemo(() => {
    ...
    return showtimes.filter((showtime) => {
      const existingRoomId = getShowtimeRoomId(showtime);
      const existingStart = new Date(showtime.startTime);
      const existingEnd = new Date(showtime.endTime);
      const existingOccupiedEnd = addMinutes(existingEnd, CLEANUP_MINUTES);
  ...
  const disableSubmit =
    isSubmitting || !hasTimeValues || (hasManualTimeError && !hasConflict);
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    if (hasConflict) {
      ...
      const nextSlot = findNextAvailableSlotAfterConflict({
        showtimes,
        roomId: values.roomId,
        preferredStartTime: values.startTime,
        movieDurationMinutes: durationMinutes,
        editingShowtimeId: editingShowtime?.id,
      });

      if (!nextSlot.ok) {
        toast.error(nextSlot.message || "Không thể tự chọn giờ chiếu phù hợp");
        return;
      }

      form.setValue("startTime", toDateTimeLocalValue(nextSlot.startDate), {
        shouldDirty: true,
        shouldValidate: true,
      });

      form.setValue("endTime", toDateTimeLocalValue(nextSlot.endDate), {
        shouldDirty: true,
        shouldValidate: true,
      });

      toast.success(
        `Bị trùng lịch nên đã tự chuyển sang ${formatTimeOnly(
          nextSlot.startDate,
        )} - ${formatTimeOnly(nextSlot.endDate)}`,
      );
  ...
  const handleTemplateNameChange = (value) => {
    ...
    setTemplateValidationErrors((current) => ({
      ...current,
      general: [],
    }));
  };

  const handleTemplateItemChange = (itemId, key, value) => {
    setTemplateDraft((current) => ({
      ...current,
  ...
  const handleAddTemplateItem = () => {
    setTemplateDraft((current) => ({
      ...current,
      items: [...current.items, createTemplateItem()],
    }));
  };
  ...
  const handleSaveTemplate = () => {
    const validation = validateScheduleTemplateDraft({
      draft: templateDraft,
      movies,
      rooms,
    });

    setTemplateValidationErrors(validation);

    if (!validation.valid) {
      const firstGeneralError = validation.general[0];
      const firstItemError = Object.values(validation.itemErrors)[0]?.[0];

      toast.error(
        firstGeneralError || firstItemError || "Mẫu lịch chưa hợp lệ",
      );
      return;
    }
    ...
    const nextTemplates = [...scheduleTemplates, normalizedTemplate];

    setScheduleTemplates(nextTemplates);
    saveScheduleTemplates(nextTemplates);
    setApplyTemplateId(normalizedTemplate.id);
    setTemplateValidationErrors({
      general: [],
      itemErrors: {},
    });
    ...
    toast.success("Đã lưu mẫu lịch chiếu cả ngày");
  };

  const handleDeleteTemplate = (templateId) => {
    const nextTemplates = scheduleTemplates.filter(
    ...
    if (String(applyTemplateId) === String(templateId)) {
      setApplyTemplateId("");
    }

    toast.success("Đã xóa mẫu lịch chiếu");
  };

  const buildPayloadsFromTemplate = ({ template, targetDate }) => {
    const errors = [];
    const candidates = [];
    ...
    template.items.forEach((item, index) => {
      ...
      if (!movie) {
        errors.push(`Dòng ${index + 1}: Không tìm thấy phim`);
        return;
      }
      ...
      if (!durationMinutes) {
        errors.push(`Dòng ${index + 1}: Phim chưa có thời lượng`);
        return;
      }

      const startDate = buildDateTimeFromDateAndTime(
        targetDate,
        item.startTime,
      );

      if (!startDate) {
        errors.push(`Dòng ${index + 1}: Giờ bắt đầu không hợp lệ`);
        return;
      }

      const endDate = addMinutes(startDate, durationMinutes);

      if (toDateInputValue(endDate) !== targetDate) {
        errors.push(
          `Dòng ${index + 1}: Suất chiếu bị qua ngày hôm sau, vui lòng chọn giờ khác`,
        );
        return;
      }
      ...
      candidates.push({
        ...
        startDate,
        endDate,
        occupiedEndDate: addMinutes(endDate, CLEANUP_MINUTES),
        payload: {
    ...
    const existingSameDate = showtimes.filter((showtime) =>
      isSameLocalDate(showtime.startTime, targetDate),
    );

    candidates.forEach((candidate) => {
      const conflictedExisting = existingSameDate.find((showtime) => {
        const existingRoomId = getShowtimeRoomId(showtime);

        if (existingRoomId !== String(candidate.item.roomId)) {
          return false;
        }
        ...
        return rangesOverlap(
          candidate.startDate,
          candidate.occupiedEndDate,
          existingStart,
          existingOccupiedEnd,
        );
      });

      if (conflictedExisting) {
        errors.push(
          `Dòng ${candidate.index + 1}: Bị trùng với suất #${
            conflictedExisting.id
          } trong cùng phòng`,
        );
      }

      const conflictedCandidate = candidates.find((other) => {
        if (other === candidate) return false;

        if (String(other.item.roomId) !== String(candidate.item.roomId)) {
          return false;
        }
        ...
        return rangesOverlap(
          ...
          candidate.occupiedEndDate,
          other.startDate,
          other.occupiedEndDate,
        );
      });

      if (conflictedCandidate) {
        errors.push(
          `Dòng ${candidate.index + 1}: Bị trùng với dòng ${
            conflictedCandidate.index + 1
          } trong mẫu`,
        );
      }
    });
    ...
    return {
      errors,
      payloads: candidates.map((candidate) => candidate.payload),
    };
  };

  const handleApplyTemplate = async () => {
    if (!selectedApplyTemplate) {
      toast.error("Vui lòng chọn mẫu lịch chiếu");
      return;
    }

    if (!applyTemplateDate) {
      toast.error("Vui lòng chọn ngày muốn tạo lịch");
      return;
    }

    const { errors, payloads } = buildPayloadsFromTemplate({
      template: selectedApplyTemplate,
      targetDate: applyTemplateDate,
    });
    ...
    if (!payloads.length) {
      toast.error("Mẫu này chưa có suất chiếu hợp lệ");
      return;
    }
    ...
    try {
      ...
      toast.success(`Đã tạo ${payloads.length} suất chiếu từ mẫu`);
    } catch (error) {
      toast.error(
        error?.response?.data?.message ||
          "Không thể tạo lịch chiếu từ mẫu đã chọn",
      );
    } finally {
    ...
    <PermissionGate permissions={[PERMISSIONS.SHOWTIMES_VIEW]}>
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
                Tạo lịch chiếu cả ngày từ mẫu
              </h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Tạo sẵn một list suất chiếu trong ngày. Lần sau chỉ cần chọn
                ngày và bấm tạo, hệ thống sẽ tự tạo toàn bộ suất chiếu.
              </p>
            </div>

            <div className="flex flex-col gap-3 md:flex-row md:items-end">
              <div className="min-w-[220px]">
                <label className="label">Mẫu lịch</label>
                <select
                  className="input"
                  value={applyTemplateId}
                  onChange={(e) => setApplyTemplateId(e.target.value)}
                >
                  <option value="">Chọn mẫu lịch chiếu</option>
                  {scheduleTemplates.map((template) => (
                    <option key={template.id} value={template.id}>
                      {template.name} · {template.items.length} suất
                    </option>
                  ))}
                </select>
              </div>
              ...
              <button
                ...
                disabled={
                  !applyTemplateId || !applyTemplateDate || isApplyingTemplate
                }
              >
                {isApplyingTemplate ? "Đang tạo lịch..." : "Tạo lịch từ mẫu"}
              </button>
          ...
          {selectedApplyTemplate ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-semibold text-slate-900 dark:text-white">
                    {selectedApplyTemplate.name}
                  </p>
                  <p className="text-sm text-slate-500">
                    Có {selectedApplyTemplate.items.length} suất chiếu trong mẫu
                  </p>
                </div>
                ...
                <button
                  type="button"
                  className="btn-danger"
                  onClick={() => handleDeleteTemplate(selectedApplyTemplate.id)}
                >
                  Xóa mẫu
                </button>
              </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {selectedApplyTemplate.items.map((item, index) => {
                  const movie = movies.find(
                    (movieItem) =>
                      String(movieItem.id) === String(item.movieId),
                  );
                  const room = rooms.find(
                    (roomItem) => String(roomItem.id) === String(item.roomId),
                  );
                  ...
                  return (
                    <div
                      key={item.id}
                      className="rounded-2xl bg-white p-4 text-sm ring-1 ring-slate-200 dark:bg-slate-950 dark:ring-slate-800"
                    >
                      <p className="font-semibold text-slate-900 dark:text-white">
                      ...
                      <p className="text-slate-500">
                        {room?.Cinema?.name || "Rạp"} · {room?.name || "Phòng"}
                      </p>
                      <p className="mt-1 text-slate-500">
                        Giá: {formatCurrency(item.basePrice || 0)}
                      </p>
                    </div>
                  );
                })}
                  ...
                  <div className="mt-4 grid gap-4">
                    ...
                    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">
                      ...
                      <ul className="mt-2 space-y-1">
                        ...
                        <li>
                          - Nếu bị trùng lịch, hệ thống có thể tự đẩy sang khung
                          giờ trống gần nhất.
                        </li>
                      </ul>
                    </div>

                    {hasTimeValues && hasManualTimeError && !hasConflict ? (
                      <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-300">
                        <p className="font-semibold">
                          Khung giờ đang nhập chưa hợp lệ
                        </p>
                    ...
                    {hasTimeValues && hasConflict ? (
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">
                        <p className="font-semibold">Khung giờ đang bị trùng</p>
                        <p className="mt-1">
                          Bấm <strong>Tự chọn giờ trống gần nhất</strong>, hệ
                          thống sẽ tự chuyển suất chiếu sang sau lịch bị trùng,
                          có tính thêm {CLEANUP_MINUTES} phút dọn vệ sinh.
                        </p>
                      </div>
                    ) : null}
                ...
                {hasConflict && (
                  <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-700 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-300">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold">
                          Khung giờ này đang bị trùng trong cùng phòng chiếu
                        </p>

                        <p className="mt-1 text-sm opacity-90">
                          Bấm nút <strong>Tự chọn giờ trống gần nhất</strong>,
                          hệ thống sẽ tự chuyển sang sau suất chiếu bị trùng, có
                          cộng thêm {CLEANUP_MINUTES} phút dọn vệ sinh.
                        </p>
        ...
        <Modal
          ...
          title="Tạo mẫu lịch chiếu cả ngày"
          subtitle="Lưu một list suất chiếu theo giờ trong ngày. Khi áp dụng mẫu, hệ thống sẽ tự ghép với ngày đã chọn."
          width="max-w-7xl"
          footer={
            <div className="flex justify-end gap-3">
              <button
          ...
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="Ví dụ: Lịch cuối tuần phòng 1, Lịch ngày thường..."
              />

              {templateValidationErrors.general?.length ? (
                <div className="mt-2 space-y-1">
                  {templateValidationErrors.general.map((error) => (
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-slate-900 dark:text-white">
                    Danh sách suất chiếu trong mẫu
                  </h3>
                  <p className="text-sm text-slate-500">
                    Mỗi dòng là một suất chiếu. Giờ kết thúc sẽ tự tính theo
                    thời lượng phim.
                  </p>
                </div>
              ...
              <div className="mt-4 space-y-3">
                {templateDraft.items.map((item, index) => {
                  const currentItemErrors =
                    templateValidationErrors.itemErrors?.[item.id]?.filter(
                      Boolean,
                    ) || [];
                  ...
                  return (
                    <div
                      key={item.id}
                      className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 lg:grid-cols-[1.2fr_1.2fr_140px_140px_140px_auto]"
                    >
                      <div>
                        <label className="label">Phim #{index + 1}</label>
                        <select
                          className="input"
                          value={item.movieId}
                          onChange={(e) =>
                        ...
                        >
                          ...
                          {movies.map((movie) => (
                            <option key={movie.id} value={movie.id}>
                              {movie.title} · {movie.durationMinutes || 0} phút
                            </option>
                          ))}
                        </select>
                      </div>
                        ...
                        >
                          ...
                          {rooms.map((room) => (
                            <option key={room.id} value={room.id}>
                              {room.Cinema?.name || "Rạp"} - {room.name}
                            </option>
                          ))}
                        </select>
                      </div>
                      ...
                      {currentItemErrors.length ? (
                        <div className="lg:col-span-6">
                          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-300">
                            <p className="font-semibold">
                              Dữ liệu dòng này chưa hợp lệ
                            </p>

                            <ul className="mt-1 list-disc space-y-1 pl-5">
                              {currentItemErrors.map((error) => (
